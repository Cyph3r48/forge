...
| Review iterations (fix attempts) | Review → fix loops per PR before PASS; capped at 2, then escalates. | Count of review-state re-entries on the Paperclip `:3100` issue; review-run outputs carry the parsed greploop verdict — verify against stubs in Wave 3. | calibration |
