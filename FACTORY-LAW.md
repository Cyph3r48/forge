...
- **Reviewer** — runs Review: uses OpenCodeReview delegation to enumerate the current diff and applicable rules, reads and judges every file in both the reviewable and excluded lists plus Tester proof, then issues PASS or FAIL with blockers named. It never fixes or approves a diff it authored or changed. PASS requires 100% review coverage, zero skipped files, and zero unresolved findings; after two Builder fix attempts, the issue escalates. Loads: piv-review-pr, rules-check-drift, open-code-review-delegate, before-and-after.
...
3. **Gates that can be code must be code** — planned scripts will check evidence, successful checks, current-commit lineage, OpenCodeReview file accounting, and the independent Reviewer verdict. Empty is not pass.
...
5. **Gates block stages** — planned gates require Tester proof before Review, Reviewer PASS with 100% OpenCodeReview coverage, zero skipped files, and zero unresolved findings, and separate human merge and deployment approvals before Ship completes.
