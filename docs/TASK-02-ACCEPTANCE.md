...
| Review | Reviewer | Cleanup exit is accepted. Tester proof and nonempty successful checks already exist, are attached, and bind to the reviewed commit before Review entry. | Reviewer reads the current diff, judges the proof, issues PASS, and records Greptile 5/5 with zero unresolved comments. | Missing, stale, empty, or failed proof returns to Builder. Reviewer findings return to Builder. After two fix attempts, go to `needs-human`. | Change the commit after Tester evidence: Review rejects it. A Reviewer cannot alter the diff or issue PASS without current evidence and the parsed 5/5 result. |
...
- A failed review records named blockers and returns to Build. The Reviewer
  never fixes the diff. Unavailable Greptile leaves Review blocked; retry the
  same commit, then escalate with the outage reason. No substitute verdict
  grants Ship.
