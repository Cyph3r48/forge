...
Read [docs/PROGRESS.md](docs/PROGRESS.md) before starting work. It records the
current phase, completed work, known failures, checks, and the next task.
Update it at each completed task or handoff. Record what actually passed;
do not mark a phase complete because its documents exist.

Owner instructions for this project take precedence over the generic workflow
...
- Keep engine integrations unconfigured unless the owner supplies and
  authorizes a connection. Do not contact personal engines during local checks.
- Work one phase at a time. UI scope is a colorful, conventional interface
  with a couple of Three.js assets and subtle effects. No separate 2D mode.
- Read the current state first and run checks appropriate to the change.
  Avoid repeating a completed audit or starting later phases during a handoff.
